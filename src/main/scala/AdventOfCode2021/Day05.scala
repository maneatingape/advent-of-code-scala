...
object Day05:
  ...
  def parse(input: Seq[String]): Seq[Line] = input.map { line =>
  ...
  def part1(input: Seq[String]): Int = overlap(parse(input).filter(_.isOrthogonal))

  def part2(input: Seq[String]): Int = overlap(parse(input))
