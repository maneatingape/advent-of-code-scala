...
object Day14:
  ...
  def insertion(input: String, steps: Int): Long =
    val Array(head, tail) = input.trim.split("\n\n")
    val quantity = head.foldLeft[Totals](Map())((map, element) => map.increase(element.toString, 1))
    val pairs = head.sliding(2).foldLeft[Totals](Map())((map, pair) => map.increase(pair, 1))
    ...
    def step(pairs: Totals, quantity: Totals): (Totals, Totals) =
      pairs.foldLeft[(Totals, Totals)]((Map(), quantity)) { case ((pairs, quantity), (key, value)) =>
        val nextPairs = pairs.increase(key.head + rules(key), value).increase(rules(key) + key.last, value)
